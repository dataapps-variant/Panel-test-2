...
def create_landing_page(main_area):
    ...
    try:
        ...
        cache_info = {"last_bq_refresh": "--", "last_gcs_refresh": "--"}
    ...
    icarus_btn = pn.widgets.Button(name="📊 Open ICARUS Dashboard", button_type="primary", width=250)
    
    def open_icarus(event):
        main_area.clear()
        main_area.append(create_icarus_dashboard(main_area))
...
def create_icarus_dashboard(main_area):
    ...
    try:
        ...
        return pn.pane.Alert(f"Error loading date bounds: {e}", alert_type="danger")
    
    # Load plan groups
    try:
        plan_groups = load_plan_groups("Active")
        if not plan_groups["Plan_Name"]:
            return pn.pane.Alert("No active plans found.", alert_type="warning")
    except Exception as e:
        return pn.pane.Alert(f"Error loading plan groups: {e}", alert_type="danger")
    ...
    def go_back(event):
        main_area.clear()
        main_area.append(create_landing_page(main_area))
    ...
    def load_data(event):
        ...
        try:
            ...
            if not selected_plans:
                results.clear()
                results.append(pn.pane.Alert("Please select at least one Plan.", alert_type="warning"))
                return
            
            selected_metrics = metrics_check.value
            if not selected_metrics:
                results.clear()
                results.append(pn.pane.Alert("Please select at least one Metric.", alert_type="warning"))
                return
            ...
            if (df_regular is None or df_regular.empty) and (df_crystal is None or df_crystal.empty):
                results.append(pn.pane.Alert("No data found for selected filters.", alert_type="warning"))
                
        except Exception as e:
            results.clear()
            results.append(pn.pane.Alert(f"Error: {str(e)}\n\n{traceback.format_exc()}", alert_type="danger"))
    ...
    return pn.Column(
        pn.Row(back_btn, pn.Spacer(), pn.pane.HTML(f'<h3 style="color:{COLORS["text_primary"]}">ICARUS - Plan (Historical)</h3>'), pn.Spacer()),
        pn.layout.Divider(),
        filters,
...
def create_app():
    ...
    if not IMPORTS_OK:
        return pn.pane.HTML(f'''
            <div style="padding: 50px; background: #0F172A; color: #F1F5F9; min-height: 100vh;">
                <h1>Import Error</h1>
                <pre style="background: #1E293B; padding: 20px; border-radius: 8px;">{IMPORT_ERROR}</pre>
            </div>
        ''')
    
    # Main content area (will be swapped for navigation)
