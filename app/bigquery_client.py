...
def load_from_bigquery():
    ...
    try:
        ...
        log_debug(f"BigQuery client creation failed: {e}")
        raise ConnectionError(f"Failed to create BigQuery client. Check credentials and project configuration. Error: {e}")
    ...
    try:
        ...
        log_debug(f"BigQuery query failed: {e}")
        raise ConnectionError(f"Failed to query BigQuery table '{BIGQUERY_FULL_TABLE}'. Error: {e}")
...
def get_master_data():
    ...
    try:
        ...
        if bucket:
            ...
            if data is not None:
                ...
                log_debug("Loaded from GCS cache")
                return data
    except Exception as e:
        log_debug(f"GCS cache load failed: {e}")
    ...
    try:
        ...
        log_debug(f"BigQuery load failed: {e}")
        raise RuntimeError(f"Failed to load data. No cache available and BigQuery query failed. Error: {e}")
    ...
    if bucket:
        try:
            ...
            log_debug(f"Failed to save to GCS cache: {e}")
